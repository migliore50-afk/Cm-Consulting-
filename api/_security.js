...
export async function consumeRateLimit(req, scope, limit = 10, windowSeconds = 900) {
  ...
  if (current === null) {
    return { configured: false, allowed: true, remaining: null };
  }

  if (Number(current) === 1) {
    await redis(`/expire/${encodeURIComponent(key)}/${windowSeconds}`);
  }
...
function magicMatches(type, bytes) {
  ...
  if (type === 'image/png') {
    return bytes.slice(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]));
  }
  ...
  if (type === 'application/msword' || type === 'application/vnd.ms-excel') {
    return bytes.slice(0, 8).equals(Buffer.from([0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]));
  }

  if (type.includes('openxmlformats')) {
    return bytes.slice(0, 2).toString('ascii') === 'PK';
  }
...
export async function scanAttachmentBuffer({ filename, buffer, contentType }) {
  const safeType = str(contentType).toLowerCase();
  const bytes = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || []);

  if (!bytes.length) {
    return { clean: false, reason: 'file_empty', size: 0 };
  }

  if (!magicMatches(safeType, bytes)) {
    return { clean: false, reason: 'file_signature_mismatch', size: bytes.length };
  }

  if (safeType === 'application/pdf' && hasDangerousPdf(bytes)) {
    return { clean: false, reason: 'pdf_active_content', size: bytes.length };
  }

  if (safeType.includes('openxmlformats') && dangerousZipMarkers(bytes)) {
    return { clean: false, reason: 'office_macro_or_embedding', size: bytes.length };
  }
  ...
  if (!scannerUrl) {
    if (String(process.env.CM_REQUIRE_ANTIVIRUS).toLowerCase() === 'true') {
      return { clean: false, reason: 'antivirus_not_configured', size: bytes.length };
    }

    return { clean: true, size: bytes.length, engine: 'signature-heuristics' };
  }
  ...
  try {
    ...
    if (!response.ok || result.clean !== true) {
      return { clean: false, reason: 'antivirus_rejected', size: bytes.length };
    }

    return { clean: true, size: bytes.length, engine: 'external-antivirus' };
  } catch (error) {
    console.error('CM Consulting API - antivirus unavailable:', error?.message || error);
    return { clean: false, reason: 'antivirus_unavailable', size: bytes.length };
  }
}

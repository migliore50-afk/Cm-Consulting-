...
**Stato:** progetto attivo. Vedi §21 per il lavoro più recente (pagine di tipologia, navigazione, pagine Chi Siamo/FAQ, pulizia footer) e i due punti ancora aperti in fondo a quella sezione.
...
**Procedura consolidata e usata per tutto il lavoro di questo registro**: Claude legge sempre il RAW da GitHub prima di modificare, prepara il file completo o le istruzioni precise, e **Carmelo carica manualmente** via editor web GitHub o "Add file → Upload files", un commit per correzione (o un gruppo di file coerenti caricati insieme). Claude verifica poi il risultato rileggendo il RAW.
...
**Premessa vincolante, vale per sempre**: CM Consulting non può dichiarare scala/numeri/anni di esperienza/recensioni dei competitor (es. "30+ anni", "2.500+ pratiche") senza violare la trasparenza IVASS — nessuna cifra falsa va introdotta. Vale anche per "marchio registrato" (vedi §21, punto aperto 2): si scrive solo ciò che è vero e verificabile.
...
**Stato generale: la maggior parte è confermata live (verificata via RAW GitHub durante la sessione). Alcuni ultimi ritocchi al footer/pulsante torna-su sono stati consegnati a Carmelo ma non riconfermati con un nuovo fetch — verificare il RAW prima di dare per scontato che siano live. Due punti restano esplicitamente aperti, vedi in fondo.**

## Sette pagine di tipologia — contenuto reale, documenti, FAQ, step personalizzati

Confermato live via RAW GitHub in questa sessione:
...
Tutto tramite lo stesso meccanismo automatico (nessun file HTML toccato uno per uno):
...
- **Pulsante "torna su"** aggiunto su ogni pagina (compare dopo scroll, sparisce in cima) — passato per due redesign su richiesta di Carmelo: prima versione cerchio bianco in basso a sinistra (bocciata, "poco evidente"); versione attuale: **ovale dorato con scritta "↑ Torna su", centrato orizzontalmente in basso**.
...
- **Riga "Registro Unico degli Intermediari — IVASS"** rimossa dalla barra finale del footer (sotto il copyright), su richiesta di Carmelo.
- **Righe vuote "Telefono:", "Email:", "PEC:"** (residuo, senza valore) rimosse dal riquadro dati societari.

## PUNTI APERTI — non risolti, richiedono azione di Carmelo o una decisione

### 1. Foto ripetute tra le pagine di tipologia — CONFERMATO IL PROBLEMA, SOLUZIONE NON ANCORA APPLICATA
...
**Causa del blocco**: Claude non ha accesso di rete per scaricare foto da internet (bash_tool senza rete) e non può caricare materiale protetto da copyright su un sito commerciale senza licenza.

**Soluzione scelta con Carmelo**: generare 4 immagini nuove con ChatGPT (DALL-E), non foto stock. Claude ha fornito 4 prompt pronti (dogana/porto container, stretta di mano per contratti privati, facciata edificio commerciale, scambio di chiavi per rami d'azienda) — **consegnati a Carmelo, in attesa che lui generi le immagini con ChatGPT e le carichi**. Una volta ricevuti i file, vanno collegati a: le card corrispondenti in `fideiussioni.html`, e i riquadri `.cd-hero-img` in `dogane.html`, `affitti-commerciali.html`, `affitti-rami-azienda.html` (nota: `fideiussioni-contratti-privati.html` non ha mai avuto un riquadro hero-image, solo la card sulla pagina hub va corretta per quella categoria).

### 2. Riga "marchio registrato" nel footer — DECISIONE NON PRESA

Carmelo ha notato che un competitor (mondocauzioni.it) scrive "[Sito] è un marchio registrato di [nome]" in fondo alla pagina e voleva qualcosa di simile. **Confermato da Carmelo: "CM Consulting" NON è registrato come marchio presso l'UIBM** — quindi quella frase esatta non si può scrivere (sarebbe falsa e verificabile). Claude ha proposto tre alternative oneste a Carmelo, che non ha ancora scelto:
- **A)** Nessuna riga aggiuntiva — il copyright già presente in fondo basta
- **B)** Una riga ancorata all'iscrizione RUI reale ("CM Consulting è l'attività di intermediazione assicurativa di Carmelo Migliore, iscritto al RUI — Sezione E, n. E000437237")
- **C)** Una riga di proprietà semplice, senza la parola "marchio" ("CM Consulting è gestito da Carmelo Migliore, iscritto al Registro Unico degli Intermediari")

**Non implementare nessuna delle tre finché Carmelo non sceglie esplicitamente.**
